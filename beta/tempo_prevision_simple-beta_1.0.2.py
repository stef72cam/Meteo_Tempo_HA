...
def shrink_probs(pB: float, pW: float, pR: float, lam: float):
    """
    Aplatit les probas vers l'uniforme (1/3,1/3,1/3).
    lam = 0.0 => inchangé ; lam = 1.0 => totalement uniforme.
    """
    u = 1.0 / 3.0
    pB2 = (1 - lam) * pB + lam * u
    pW2 = (1 - lam) * pW + lam * u
...
def mask_probs_by_calendar(d, pB: float, pW: float, pR: float):
    """
    Met à 0 les couleurs impossibles (dimanche, samedi rouge interdit, etc.)
    puis renormalise.
    """
    wd = d.weekday()

    # Dimanche = bleu certain (zéro débat)
    if wd == 6:
        return 1.0, 0.0, 0.0
    ...
    s = pB + pW + pR
    if s <= 0:
        # fallback safe : bleu/blanc si possible
        if wd == 5:
...
def build_forecast(real_j1=REAL_J1):
    ...
    for entry in days:
        ...
        # ---------------------------------------------------------
        # CALIBRATION J+4..J+6 (incertitude ++, surtout si ENR non fiable)
        # ---------------------------------------------------------
